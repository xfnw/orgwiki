...
#+begin_chat hi
also consider using sequoia sq instead of gpg: most of these
recommendations are sq's default behavior
#+end_chat

* Curve25519 keys
Curve25519 is now the default! yay!
...
[[file:gpg-backup.org::*just the subkeys][only keep the easily-rotatable (and revocable) subkeys on your
daily-driver computer]].

gpg usually hides the option to make keys with only Certify when
generating keys, use ~--full-generate-key --expert~ to see that

then, to add subkeys with other capabilities, run ~--edit-key~ on your
key id, and use the ~addkey~ command. ~save~ when done.
...
automatically makes revocation certificates that have an unspecified
reason and can be used in a pinch, however +it is better to have ones
that have reasons+.
...
or maybe not? anecdotally, i have had trouble with getting some
keyservers to accept revocation certificates that specify a reason,
but maybe i was just doing something wrong
#+end_chat
