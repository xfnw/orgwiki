...
since procfs options are not persistent over reboots, it can be
enabled persistently with the sysctl option ~kernel.sysrq=1~ in
...
| key | behavior                                                                 |
|-----+--------------------------------------------------------------------------|
| f   | force an [[https://en.wikipedia.org/wiki/Out_of_memory#Recovery][OOM kill]] now, even if not out of memory. may help with a freeze |
...
| e   | sigterm (graceful kill) all tasks                                       |
| i   | sigkill all tasks                                                        |
| s   | sync disks                                                               |
