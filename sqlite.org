...
* foreign keys
to enforce a dependency on another table, foreign keys can be used.
...
that was chosen by an aggregate function. it does weird stuff if you
have multiple aggregate functions though
